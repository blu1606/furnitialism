            ...
            intervalRef.current = setInterval(() => {
                if (currentLineIdx.current < FORGE_MESSAGES.length) {
                    ...
                    if (currentLineIdx.current === FORGE_MESSAGES.length) {
                        ...
                        setTimeout(() => {
                            setShowForge(false);
                            // We set standalone view with a realistic prototype ID
                            setStandaloneView('VOXLÖV-PROTOTYPE');
                        }, 2000);
                    }
                } else {
                    clearInterval(intervalRef.current);
        ...
        <div className={`forge-overlay ${showForge ? 'active' : ''}`}>
            <div className="forge-window glass-panel">
                ...
                <div className="forge-main">
                    ...
                    {forgeStep === 0 ? (
                        <div className="upload-zone-premium" onClick={handleUpload}>
                            {isUploading ? (
                                <div className="initializing-spinner">
                                    <div className="spinner-ring"></div>
                                    <span className="hub-label" style={{ color: '#000', marginTop: '20px' }}>INITIALIZING ENGINE...</span>
                                </div>
                            ) : (
                                <>
                                    <div className="upload-icon-wrapper">
                                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" strokeWidth="1.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" y1="3" x2="12" y2="15" /></svg>
                                    </div>
                                    <div className="upload-text">
                                        <span className="hub-label" style={{ fontSize: '1.2rem', color: '#000', display: 'block' }}>Drop Reference Image</span>
                                        <span style={{ fontSize: '0.85rem', opacity: 0.5, color: '#000' }}>AI Reconstructs in Seconds</span>
                                    </div>
                                    <button className="forge-action-btn">BROWSE FILES</button>
                                </>
                            )}
                        </div>
                    ) : (
                        <div className="terminal-container">
                            <div className="terminal-header">
                                ...
                                <span className="terminal-title">RECONSTRUCTION_ENGINE_V4.0</span>
                            </div>
                            <div className="terminal-output">
                                {terminalLines.map((line, i) => (
                                    <div key={i} className="terminal-line" style={{ display: 'flex', gap: '15px' }}>
                                        ...
                                        <span className={`line-text ${line && line.step === forgeStep ? 'active-step' : ''}`}>
                                            {line ? line.text : ''}
                                        </span>
                                    </div>
                                ))}
